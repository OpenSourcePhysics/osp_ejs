...
public class DrawingPanel3D extends InteractivePanel implements InteractionSource {
	...
	protected boolean autoscaleZ = false, removeHiddenLines = true, respondToMouse = true, showPosition = true,
			useColorDepth = true;
	...
	private double cosAlpha = Math.cos(alpha), sinAlpha = Math.sin(alpha), cosBeta = Math.cos(beta),
	...
	public DrawingPanel3D(int _displayMode) {
		...
		xAxis.setResolution(axesRes);
		xAxis.setEnabled(false);
		xAxis.canBeMeasured(false);
		xText = new InteractiveText("X");
		xText.getStyle().setFont(new Font("Dialog", Font.PLAIN, 12));
		xText.setEnabled(false);
		xText.canBeMeasured(false);
		yAxis = new InteractiveArrow(InteractiveArrow.ARROW);
		yAxis.setResolution(axesRes);
		yAxis.setEnabled(false);
		yAxis.canBeMeasured(false);
		yText = new InteractiveText("Y");
		yText.getStyle().setFont(new Font("Dialog", Font.PLAIN, 12));
		yText.setEnabled(false);
		yText.canBeMeasured(false);
		zAxis = new InteractiveArrow(InteractiveArrow.ARROW);
		zAxis.setResolution(axesRes);
		zAxis.setEnabled(false);
		zAxis.canBeMeasured(false);
		zText = new InteractiveText("Z");
		zText.getStyle().setFont(new Font("Dialog", Font.PLAIN, 12));
		zText.setEnabled(false);
		zText.canBeMeasured(false);
		// Create the trackers
		...
		for (int i = 0, n = trackerLines.length; i < n; i++) {
			...
			trackerLines[i].setResolution(axesRes);
			trackerLines[i].setEnabled(false);
			trackerLines[i].setVisible(false);
			trackerLines[i].canBeMeasured(false);
		}
...
// ------------------------------------------------
	...
	/**
	 * Whether to allow quick redraws when rotating the scene. This is independent
	 * of the value of removeHiddenLines.
	 * 
	 * @param value the desired value
	 */
	 ...
	 */
	public void setXFormat(java.text.NumberFormat _format) {
	 ...
	 * @param _format the desired format
	 */
	public void setYFormat(java.text.NumberFormat _format) {
	 ...
	 * @param _format the desired format
	 */
	public void setZFormat(java.text.NumberFormat _format) {
	 ...
	 * <li><b>CURSOR_CROSHAIR</b>: Lines from the minima to the maxima through the
	 * cursor point</li>
	 * </ul>
	 * 
	 * @param value the desired value
	...
	public void setDecorationType(int _value) { // Can't save time (see setDisplayMode)
		...
		case DECORATION_AXES:
			boolean showX = displayMode == DISPLAY_PLANAR_XY || displayMode == DISPLAY_PLANAR_XZ
					|| displayMode >= DISPLAY_3D;
			boolean showY = displayMode == DISPLAY_PLANAR_XY || displayMode == DISPLAY_PLANAR_YZ
					|| displayMode >= DISPLAY_3D;
			boolean showZ = displayMode == DISPLAY_PLANAR_YZ || displayMode == DISPLAY_PLANAR_XZ
					|| displayMode >= DISPLAY_3D;
			xAxis.setVisible(showX);
			xText.setVisible(showX);
			yAxis.setVisible(showY);
			yText.setVisible(showY);
			zAxis.setVisible(showZ);
			zText.setVisible(showZ);
			for (int i = 0, n = boxSides.length; i < n; i++)
				boxSides[i].setVisible(false);
			break;
	...
	public void setDecorationResolution(Resolution _res) {
		for (int i = 0, n = boxSides.length; i < n; i++)
			boxSides[i].setResolution(_res);
		xAxis.setResolution(_res);
		yAxis.setResolution(_res);
		zAxis.setResolution(_res);
		for (int i = 0, n = trackerLines.length; i < n; i++)
			trackerLines[i].setResolution(_res);
	}
	...
	/**
	 * Get the displacement of the center of the scene with respect to the center of
	 * the panel
	 * 
	 * @return the current displacement
	 */
	public java.awt.Point getPan() {
		return new java.awt.Point(this.deltaa, this.deltab);
	}

	/**
	 * Set the angle (in radians) to rotate the scene horizontally before
	 * projecting. Default is 0.0.
	 * 
	...
	/**
	 * Get the angle (in degrees) to rotate the scene horizontally before
	 * projecting.
	 * 
	 * @return the current value
	 */
	...
	/**
	 * Set the angle (in radians) to rotate the scene vertically before projecting.
	 * Default is 0.0.
	 * 
	...
	/**
	 * Set the angles (in radians) to rotate the scene horizontally and vertically
	 * before projecting
	 * 
	...
	/**
	 * Set the ratio between the distance to the user's eye and the center of the
	 * scene
	 * 
	...
	/**
	 * Get the ratio between the distance to the user's eye and the center of the
	 * scene
	 * 
	 * @return the current value
	 */
	...
	/**
	 * Set the ratio between the distance to the user's eye and the projecting plane
	 * (actually, the screen)
	 * 
	 * @param _value the desired value
	 */
	...
	/**
	 * Get the ratio between the distance to the user's eye and the projecting plane
	 * (actually, the screen)
	 * 
	 * @return the current value
	 */
...
// ------------------------------------------------
	...
	/**
	 * Limits the zmin and zmax values during autoscaling so that the mininimum
...
// -------------------------------------
	...
	public void addListener(InteractionListener _listener) {
		if (_listener == null || listeners.contains(_listener))
			return;
...
// -------------------------------------
	...
	public void setForeground(Color _color) {
		...
		if (xAxis != null) {
			for (int i = 0, n = boxSides.length; i < n; i++)
				boxSides[i].getStyle().setEdgeColor(_color);
			xAxis.getStyle().setEdgeColor(_color);
			xText.getStyle().setEdgeColor(_color);
			yAxis.getStyle().setEdgeColor(_color);
			yText.getStyle().setEdgeColor(_color);
			zAxis.getStyle().setEdgeColor(_color);
			zText.getStyle().setEdgeColor(_color);
			for (int i = 0, n = trackerLines.length; i < n; i++)
				trackerLines[i].getStyle().setEdgeColor(_color.brighter().brighter());
		}
	}
...
//      }
//    }
		if (xm != xmin || xM != xmax || ym != ymin || yM != ymax || zm != zmin || zM != zmax)
			computeConstants(9); // Saves time
	}
	...
	public void clear() {
		...
		for (int i = 0, n = boxSides.length; i < n; i++)
			this.addDrawable(boxSides[i]);
		this.addDrawable(xAxis);
		this.addDrawable(xText);
		this.addDrawable(yAxis);
		this.addDrawable(yText);
		this.addDrawable(zAxis);
		this.addDrawable(zText);
		for (int i = 0, n = trackerLines.length; i < n; i++)
			this.addDrawable(trackerLines[i]);
	}
	...
	public void paintDrawableList(Graphics g, ArrayList<Drawable> tempList) {
		...
		} else { // synchronized(list3D) { // Collect objects, sort and draw them one by one.
					// Takes time!!!
			list3D.clear();
			while (it.hasNext()) {
				if (!isValidImage()) {
				...
				if (drawable instanceof Drawable3D) {
					...
					drawable.draw(this, g2); // Normal drawables are drawn BEFORE Drawables3D
			}
			if (list3D.size() > 0) {
	...
	/**
	 * Convert a 3D point of the scene into a 2D point of the screen. It can compute
	 * simultaneously the equivalent in screen coordinates of a distance in the
	 * scene. Finally, it also provides a number measuring the relative distance of
	 * the point to the useTODr's viewpoint. distance = 1.0 means at the center of
	 * the scene, distance > 1.0 means farther than the center of the scene,
	 * distance < 1.0 means closer than the center of the scene,
	 * 
	 * @param coordinate The coordinates of the point of the scene <itemize>
	 *                   <li>If the input array has length 2, it is considered a
	 *                   single 2D point x,y
	 *                   <li>If the input array has length 3, it is considered a
	 *                   single 3D point x,y,z
	 *                   <li>If the input array has length 4, it is considered a 2D
	 *                   point plus a 2D vector x,y, dx,dy
	 *                   <li>If the input array has length 6, it is considered a 3D
	 *                   point plus a 3D vector x,y,z, dx,dy,dz </itemize>
	 * @param pixel      a place-holder for the coordinates of the point of the
	 *                   screen <itemize>
	 *                   <li>If the input array had length 2 or 3, it returns a,b
	 *                   and the distance
	 *                   <li>If the input array had length 4 or 6, it returns a,b,
	 *                   da,db and the distance </itemize>
	...
	public double[] project(double[] coordinate, double[] pixel) {
		...
		switch (displayMode) {
		case DISPLAY_PLANAR_XY:
			xprime = z;
			yprime = x;
			zprime = y;
			factor = 1.8;
			break;
		case DISPLAY_PLANAR_XZ:
			xprime = y;
			yprime = x;
			zprime = z;
			factor = 1.8;
			break;
		case DISPLAY_PLANAR_YZ:
			xprime = x;
			yprime = y;
			zprime = z;
			factor = 1.8;
		...
		case DISPLAY_PERSPECTIVE:
			...
			if (Math.abs(aux) < 1.0e-2)
				aux = 1.0e-2; // This is to avoid division by zero
		...
		case 6: // Input is x,y,z,dx,dy,dz
			...
			case DISPLAY_PLANAR_XY:
				...
				break; // dx,dy are used
			case DISPLAY_PLANAR_XZ:
				pixel[2] = coordinate[3] * factor * aconstant;
				pixel[3] = coordinate[5] * factor * bconstant;
				break; // dx,dz are used
			case DISPLAY_PLANAR_YZ:
				pixel[2] = coordinate[4] * factor * aconstant;
				pixel[3] = coordinate[5] * factor * bconstant;
				break; // dy,dz are used
			default:
				/* 3D */ pixel[2] = Math.max(coordinate[3], coordinate[4]) * factor * aconstant;
				pixel[3] = coordinate[5] * factor * bconstant;
				break;
...
// -------------------------------------
	...
	/**
	 * Compute the display color of a given drawable3D based on its original color
	 * and its depth. Transparency of the original color is not affected.
	 * 
...
//    else return _aColor;
	...
	/**
	 * Converts a point on the screen into a world point Private because it only
	 * works properly for planar display modes
	 */
	private Point3D worldPoint(int a, int b) {
		double factor = 1.8;
		switch (displayMode) {
		case DISPLAY_PLANAR_XY:
			return new Point3D(centerX + (a - acenter) / (factor * aconstant),
					centerY + (bcenter - b) / (factor * bconstant), zmax);
		case DISPLAY_PLANAR_XZ:
			return new Point3D(centerX + (a - acenter) / (factor * aconstant), ymax,
					centerZ + (bcenter - b) / (factor * bconstant));
		case DISPLAY_PLANAR_YZ:
			return new Point3D(xmax, centerY + (a - acenter) / (factor * aconstant),
					centerZ + (bcenter - b) / (factor * bconstant));
		default:
			/* 3D */ return new Point3D((xmin + xmax) / 2, (ymin + ymax) / 2, (zmin + zmax) / 2);
		}
	}

	/**
	 * Converts into a world distance a distance on the screen Private because it
	 * only works properly for planar display modes
	 */
	private Point3D worldDistance(int dx, int dy) {
		double factor = 1.8;
		switch (displayMode) {
		case DISPLAY_PLANAR_XY:
			return new Point3D(dx / (factor * aconstant), -dy / (factor * bconstant), 0.0);
		case DISPLAY_PLANAR_XZ:
			return new Point3D(dx / (factor * aconstant), 0.0, -dy / (factor * bconstant));
		case DISPLAY_PLANAR_YZ:
			return new Point3D(0.0, dx / (factor * aconstant), -dy / (factor * bconstant));
		default:
			/* 3D */ return new Point3D(dx / (1.3 * aconstant), dy / (1.3 * bconstant), 0.0);
		}
	}
...
//  public int getKeyPressed () { return keyPressed; }
	...
	private boolean mouseDraggedComputations(java.awt.event.MouseEvent e) {
		if (e.isControlDown()) {
			setPan(deltaa + (e.getX() - lastX), deltab + (e.getY() - lastY));
			return false;
		} // Panning
		else if (e.isShiftDown()) {
			setZoom(zoom - (e.getY() - lastY) * 0.01);
			return false;
		} // Zooming
		...
		if (displayMode < DISPLAY_3D) {
			switch (keyPressed) {
			case 88:
				trackerPoint.x += point.x;
				break; // X is pressed
			case 89:
				trackerPoint.y += point.y;
				break; // Y is pressed
			case 90:
				trackerPoint.z += point.z;
				break; // Z is pressed
			default:
				trackerPoint.x += point.x;
				trackerPoint.y += point.y;
			...
			case 88: // X is pressed
				if (cosAlpha >= 0 && Math.abs(sinAlpha) < cosAlpha)
					trackerPoint.x += point.y;
				else if (sinAlpha >= 0 && Math.abs(cosAlpha) < sinAlpha)
					trackerPoint.x -= point.x;
				else if (cosAlpha < 0 && Math.abs(sinAlpha) < -cosAlpha)
					trackerPoint.x -= point.y;
				else
					trackerPoint.x += point.x;
				break;
...
// ------------------------------------------
	...
	/*
	 * TODO : overwrite findInteractive so that it checks first objects that are
	 * closer
	 */
	...
	private void resetInteraction(InteractivePanel _panel) {
		...
		targetHit = null;
		showTrackers(false);
		messages.setMessage(this, null, 0);
		repaint();
	}
	...
	public void handleMouseAction(InteractivePanel _panel, java.awt.event.MouseEvent _evt) {
		...
		case InteractivePanel.MOUSE_PRESSED:
			requestFocus();
			if (_evt.isPopupTrigger() || _evt.getModifiers() == java.awt.event.InputEvent.BUTTON3_MASK)
				return;
...
//          displayTrackerPosition(); // Will be changed too quickly to see this one
				targetHit.getSource().invokeActions(
						new InteractionEvent(targetHit.getSource(), InteractionEvent.MOUSE_PRESSED, null, targetHit));
//          invokeActions (new InteractionEvent (this,InteractionEvent.MOUSE_PRESSED,null,targetHit));
				trackerPoint = targetHit.getHotspot(_panel); // because the listener may change the position of the
																// element
				...
				default: /* 3D */
				case DISPLAY_PLANAR_XY:
					trackerPoint.x = iad.getX();
					trackerPoint.y = iad.getY();
					trackerPoint.z = zmax;
					break;
				case DISPLAY_PLANAR_XZ:
					trackerPoint.x = iad.getX();
					trackerPoint.z = iad.getY();
					trackerPoint.y = ymax;
					break;
				case DISPLAY_PLANAR_YZ:
					trackerPoint.y = iad.getX();
					trackerPoint.z = iad.getY();
					trackerPoint.x = xmax;
					break;
				}
			...
			if (showPosition)
				displayPosition(trackerPoint.x, trackerPoint.y, trackerPoint.z);
			positionTrackers();
		...
		case InteractivePanel.MOUSE_DRAGGED:
			if (_evt.isPopupTrigger() || _evt.getModifiers() == java.awt.event.InputEvent.BUTTON3_MASK)
				return;
			if (iad != null && iad.isEnabled() == false)
				return;
			...
			if (targetHit != null) {
				...
				targetHit.getSource().invokeActions(
						new InteractionEvent(targetHit.getSource(), InteractionEvent.MOUSE_DRAGGED, null, targetHit));
//          invokeActions (new InteractionEvent (this,InteractionEvent.MOUSE_DRAGGED,null,targetHit));
				...
				invokeActions(new InteractionEvent(this, InteractionEvent.MOUSE_DRAGGED, null, iad));
			else
				invokeActions(new InteractionEvent(this, InteractionEvent.MOUSE_DRAGGED, null, trackerPoint));
			if (showPosition)
				displayPosition(trackerPoint.x, trackerPoint.y, trackerPoint.z);
			positionTrackers();
			showTrackers(true); // should trackers appear only in 3D mode?
		...
		case InteractivePanel.MOUSE_RELEASED:
			...
			if (targetHit != null) {
				targetHit.getSource().invokeActions(
						new InteractionEvent(targetHit.getSource(), InteractionEvent.MOUSE_RELEASED, null, targetHit));
//          invokeActions (new InteractionEvent (this,InteractionEvent.MOUSE_RELEASED,null,targetHit));
			} else if (iad != null)
				invokeActions(new InteractionEvent(this, InteractionEvent.MOUSE_RELEASED, null, iad));
			else {
...
//        quickRedrawOn = false;
		...
		case InteractivePanel.MOUSE_MOVED: // TODO : Should panel invoke actions here?
			if (_panel.getInteractive() != null)
				_panel.setMouseCursor(java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR));
			else
				_panel.setMouseCursor(java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.CROSSHAIR_CURSOR));
...
//    System.out.println ("zminPreferred="+zminPreferred+", zmaxPreferred="+zmaxPreferred);
		...
		acenter = deltaa + thisWidth / 2;
		bcenter = deltab + thisHeight / 2;
		if (squareAspect)
		...
		case DISPLAY_PLANAR_YZ:
			...
			break;
		default:
			/* 3D */ maxSpace = Math.max(Math.max(dx, dy), dz);
			break;
		}
		centerX = (xmax + xmin) / 2.0;
		centerY = (ymax + ymin) / 2.0;
		centerZ = (zmax + zmin) / 2.0;
		aconstant = 0.5 * zoom * thisWidth / maxSpace;
		bconstant = 0.5 * zoom * thisHeight / maxSpace;
	...
	private void displayPosition(double x, double y, double z) {
		switch (displayMode) {
		case DISPLAY_PLANAR_XY:
			messages.setMessage(this, xFormat.format(x) + " " + yFormat.format(y), 0);
			break;
		case DISPLAY_PLANAR_XZ:
			messages.setMessage(this, xFormat.format(x) + " " + zFormat.format(z), 0);
			break;
		case DISPLAY_PLANAR_YZ:
			messages.setMessage(this, yFormat.format(y) + " " + zFormat.format(z), 0);
			break;
		default:
			/* 3D */ messages.setMessage(this, xFormat.format(x) + " " + yFormat.format(y) + " " + zFormat.format(z),
					0);
			break;
		}
	}
...
//    if (trackerPoint.z<zmin) trackerPoint.z = zmin; else if (trackerPoint.z>zmax) trackerPoint.z = zmax;
		...
		default:
		case CURSOR_XYZ:
			trackerLines[0].setXYZ(trackerPoint.x, ymin, zmin);
			trackerLines[0].setSizeXYZ(0, trackerPoint.y - ymin, 0);
			trackerLines[1].setXYZ(xmin, trackerPoint.y, zmin);
			trackerLines[1].setSizeXYZ(trackerPoint.x - xmin, 0, 0);
			trackerLines[2].setXYZ(trackerPoint.x, trackerPoint.y, zmin);
			trackerLines[2].setSizeXYZ(0, 0, trackerPoint.z - zmin);
			break;
		case CURSOR_CUBE:
			trackerLines[0].setXYZ(xmin, trackerPoint.y, trackerPoint.z);
			trackerLines[0].setSizeXYZ(trackerPoint.x - xmin, 0, 0);
			trackerLines[1].setXYZ(trackerPoint.x, ymin, trackerPoint.z);
			trackerLines[1].setSizeXYZ(0, trackerPoint.y - ymin, 0);
			trackerLines[2].setXYZ(trackerPoint.x, trackerPoint.y, zmin);
			trackerLines[2].setSizeXYZ(0, 0, trackerPoint.z - zmin);
			trackerLines[3].setXYZ(trackerPoint.x, ymin, zmin);
			trackerLines[3].setSizeXYZ(0, trackerPoint.y - ymin, 0);
			trackerLines[4].setXYZ(xmin, trackerPoint.y, zmin);
			trackerLines[4].setSizeXYZ(trackerPoint.x - xmin, 0, 0);
			trackerLines[5].setXYZ(trackerPoint.x, ymin, zmin);
			trackerLines[5].setSizeXYZ(0, 0, trackerPoint.z - zmin);
			trackerLines[6].setXYZ(xmin, ymin, trackerPoint.z);
			trackerLines[6].setSizeXYZ(trackerPoint.x - xmin, 0, 0);
			trackerLines[7].setXYZ(xmin, trackerPoint.y, zmin);
			trackerLines[7].setSizeXYZ(0, 0, trackerPoint.z - zmin);
			trackerLines[8].setXYZ(xmin, ymin, trackerPoint.z);
			trackerLines[8].setSizeXYZ(0, trackerPoint.y - ymin, 0);
			break;
		case CURSOR_CROSSHAIR:
			trackerLines[0].setXYZ(xmin, trackerPoint.y, trackerPoint.z);
	...
	private void resetDecoration(double _dx, double _dy, double _dz) {
		if (boxSides == null || boxSides[0] == null)
			return;
		...
		yText.setXYZ(xmin, ymax + _dx * 0.02, zmin);
		zAxis.setXYZ(xmin, ymin, zmin);
		zAxis.setSizeXYZ(0.0, 0.0, _dz);
		zText.setXYZ(xmin, ymin, zmax + _dx * 0.02);
	}
...
class Comparator3D implements java.util.Comparator<Object> {
	...
	public int compare(Object o1, Object o2) {
		try {
			...
			return 0;
		} // Sometimes a NullPointerException happens
	}
}
