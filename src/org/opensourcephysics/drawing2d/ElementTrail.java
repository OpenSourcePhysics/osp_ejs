 ...
 * </p>
 * This object is often used to show the path of a moving object. Points are
 * added to a trail either with addPoint() or moveToPoint(). Trails can have
 * many segments. A segment is a section of a Trail that has a starting point, a
 * stopping point, and a color. The clear method removes all segments from the
 * trail whereas the initialize method only removes data from the current
 * segment.
 * 
...
public class ElementTrail extends Element implements org.opensourcephysics.display.Data {
	...
	private java.util.List<PathAndStyle> segmentList = new ArrayList<PathAndStyle>(); // The list of past Segments, if
																						// any
	private TrailPoint flushPoint = nullPoint; // This is the last point which was not added because of the skip
												// parameter
...
//  private int firstPoint = 0; // the first point of the current path 
	...
	/**
	 * Set/unset the active state of the trail. The trail does not use this state,
	 * but just keeps an internal value that programs can consult in order not to
	 * add data to the trail. In other words, a trail will always honor an
	 * addPoint() command. But programs can consult this internal value to decide
	 * whether or not to send data to a trail. Default value is true.
	 * 
	 * @param _active
	 */
	...
	/**
	 * Sets the no repeat state of the trail. When set, a trail will ignore (x,y)
	 * points which equal the last added point.
	 * 
	 * @param _noRepeat
	 */
	...
	/**
	 * Sets the trail to clear existing points when receiving a new point or array
	 * of points.
	 * 
	 * @param _clear
	 */
	...
	/**
	 * Sets the skip parameter. When the skip parameter is larger than zero, the
	 * trail only considers one of every 'skip' points. That is, if skip is 3, the
	 * trail will consider only every third point sent to it. The default is zero,
	 * meaning all points must be considered.
	 * 
	 * @param _skip
	 */
	...
	/**
	 * Moves to the new point without drawing. (Equivalent to setting the connection
	 * type to NO_CONNECTION and adding one single point, then setting the type back
	 ...
	 * 
	 * @param point double[][] The double[nPoints][2] array with the coordinates of
	 *              the points.
	 * @param input
	 */
	public void addPoints(double[][] input) {
		...
		for (int i = 0, n = input.length; i < n; i++)
			addPoint(input[i][0], input[i][1], this.connectionType);
	}
	...
	public void addPoints(double[] x, double y) {
		...
		for (int i = 0, n = x.length; i < n; i++)
			addPoint(x[i], y, this.connectionType);
	}
	...
	public void addPoints(double x, double[] y) {
		...
		for (int i = 0, n = y.length; i < n; i++)
			addPoint(x, y[i], this.connectionType);
	}
	...
	public void addPoints(double[] xInput, double[] yInput) {
		...
		for (int i = 0; i < n; i++)
			addPoint(xInput[i], yInput[i], this.connectionType);
	}

	/**
	 * Sets the maximum number of points for the trail. Once the maximum is reached,
	 * adding a new point will cause remotion of the first one. This is useful to
	 * keep trails down to a reasonable size, since very long trails can slow down
	 * the rendering (in certain implementations). If the value is 0 (the default)
	 * the trail grows forever without discarding old points.
	 * 
	 * @param maximum int
	 */
...
//    System.out.println ("clear");
	...
	/**
	 * Clears all points from the last segment of the trail, respecting previous
	 * segments.
	 */
	...
	public void newSegment() {
		...
		synchronized (flushPoint) {
			if (flushPoint != nullPoint)
				extraPoint = new TrailPoint(flushPoint.x, flushPoint.y, flushPoint.type);
		}
		if (extraPoint != null) {
	...
	public void draw(org.opensourcephysics.display.DrawingPanel _panel, Graphics _g) {
		...
		for (int i = 0, n = segmentList.size(); i < n; i++) {
			PathAndStyle pas = segmentList.get(i);
			Color color = pas.style.getLineColor();
			if (color != null) {
				g2.setStroke(pas.style.getLineStroke());
				g2.setColor(color);
				g2.draw(transformPath(_panel, pas.path));
			}
		}
		...
		g2.setColor(color);
		Shape tp = transformPath(_panel, getCurrentSegmentPath());
		g2.draw(tp);
...
//		Shape trShape = getPixelTransform(_panel).createTransformedShape(getCurrentSegmentPath());
		...
		if (targetPosition.isEnabled()) {
			for (PathAndStyle pas : segmentList) {
				for (TrailPoint point : pas.pointsList) {
					if (Math.abs(point.pixel[0] - _xpix) < sensitivity
							&& Math.abs(point.pixel[1] - _ypix) < sensitivity)
						return this.targetPosition;
				}
			}
			for (TrailPoint point : currentList) {
				if (Math.abs(point.pixel[0] - _xpix) < sensitivity && Math.abs(point.pixel[1] - _ypix) < sensitivity)
					return this.targetPosition;
			}
			synchronized (flushPoint) {
				if (flushPoint != nullPoint && Math.abs(flushPoint.pixel[0] - _xpix) < sensitivity
						&& Math.abs(flushPoint.pixel[1] - _ypix) < sensitivity)
					return this.targetPosition;
			}
		}
		return null;
	...
	private void projectPoints(org.opensourcephysics.display.DrawingPanel _panel) {
		...
		for (int i = 0, n = segmentList.size(); i < n; i++) {
			List<TrailPoint> list = segmentList.get(i).pointsList;
			for (int k = 0, nn = list.size(); k < nn; k++) {
				list.get(k).project(tr);
			}
		}
		for (int i = 0, n = currentList.size(); i < n; i++) {
			currentList.get(i).project(tr);
		}
		synchronized (flushPoint) {
			if (flushPoint != nullPoint)
				flushPoint.project(tr);
		}
	...
	private void addPoint(double _x, double _y, int _type) {
		// if (!active) return;
		//System.out.println("ElementTrail.addPoint " + _x + "," + _y + " " + _type);
		if (Double.isNaN(_x) || Double.isNaN(_y)) {
			isEmpty = true;
			return;
		}
		if (noRepeat && lastX == _x && lastY == _y)
			return;
		...
		synchronized (currentList) {
			...
			if (maximum > 2 && size >= maximum) {
				...
				synchronized (currentPath) {
					...
					for (int i = 0, n = currentList.size(); i < n; i++) {
						TrailPoint point = currentList.get(i);
						switch (point.type) {
						default:
						case LINE_CONNECTION:
