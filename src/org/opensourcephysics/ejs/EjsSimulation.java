...
import org.opensourcephysics.ejs.utils.FileUtils;
import org.opensourcephysics.ejs.utils.HtmlPageInfo;
import org.opensourcephysics.ejs.utils.LocaleItem;
import org.opensourcephysics.ejs.utils.LocaleListener;
...
//  static private final String EJS_TMP_SIM_DIR = EJS_TMP_DIR+"/Simulations/";
	...
	static public String getPathToLibrary() {
...
//      System.err.println ("Path set to "+_path);
	...
	static public File getResourceFile(String _filename, boolean _deleteOnExit) {
		...
		try {
			...
			System.out.println(
					"Ejs simulation: resource " + _filename + " extracted into " + resourceFile.getAbsolutePath());
			return resourceFile;
		} catch (Exception exc) {
	...
	static public File extractToDirectory(String _resourceString, File _targetDirectory, boolean _verbose,
		...
		try {
			...
			fout.close();
			if (_verbose)
				System.out.println("Ejs simulation: resource " + _resourceString + " extracted into "
...
//      System.out.println ("Must read state "+resetFile);
			if (resetFile.equals(DEFAULT_STATE_FILENAME))
...
// ---------------------------------------------------
	...
	private void errorMessage(String _text) {
		System.err.println(getClassname() + ": " + _text);
	}
	...
	/**
	 * Sets the parent component of any subsequent message window such as a
	 * JOptionPane.
	 * 
	...
	/**
	 * Gets the parent component for subsequent message windows such as a
	 * JOptionPane.
	 * 
...
// -----------------------------
	...
	public void addAvailableLocale(String _language) {
		...
		if (localeItem != null)
			availableLocales.add(localeItem);
		else
...
//    System.out.println("Setting to locale "+_language);
	...
	protected void setViewLocale() {
		...
		if (view instanceof EjsControl)
			((EjsControl) view).addElementsMenuEntries();
		view.update();
		view.finalUpdate();
	};
...
// ---------------------------------------------------
	...
	private void createDescriptionDialog(int _width, int _height) {
		...
		increaseFontButton.setToolTipText(DisplayRes.getString("DrawingFrame.IncreaseFontSize_menu_item")); // Memory.getResource("Increase
																											// font"));
							...
							for (EditorAndScroll pane : descriptionPagesList) {
								if (selectedComponent == pane.scrollPane) { // This one is
									if (!openDescriptionPageInBrowser(pane.name))
										JOptionPane.showMessageDialog(openPageButton,
												Memory.getResource("DescriptionPages.ErrorOpeningFiles"),
...
//    Dimension bounds = java.awt.Toolkit.getDefaultToolkit().getScreenSize();
	...
	public void addDescriptionPage(String _htmlPage, int _width, int _height, boolean _visible) {
		...
		if (pageInfo == null) {
			JOptionPane.showMessageDialog(popupTriggeredBy, "Html file not found: " + _htmlPage, "Description Error",
...
//    System.out.println ("Loading html page = "+pageInfo.getLink());
		...
		if (descriptionDialog == null)
			createDescriptionDialog(_width, _height);
		...
		try {
			...
			// try {
			// kit.getStyleSheet().loadRules(ResourceLoader.openReader("_ejs_library/css/ejsPage.css"),null);
			// }
			// catch (Exception exc) { exc.printStackTrace(); }
			editorPane.addHyperlinkListener(new HyperlinkListener() {
				public void hyperlinkUpdate(HyperlinkEvent e) {
					if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
						openURL(e.getSource(), e.getURL(), getView().getComponent(getMainWindow()),
								model._getApplet() != null);
					}
				}
			});
			...
			if (descriptionPagesList == null)
				descriptionPagesList = new ArrayList<EditorAndScroll>();
...
//  }
	...
	public void addDescriptionPage(String _title, String _htmlPage, int width, int height) {
		addDescriptionPage(_htmlPage, width, height, true);
	}
	...
	protected void recreateDescriptionPanel() {
		...
		for (EditorAndScroll pane : descriptionPagesList) {
			...
			if (pageInfo != null)
				descriptionPanel.add(pageInfo.getTitle(), pane.scrollPane);
		}
		descriptionDialog.pack();
			...
			for (EditorAndScroll pane : descriptionPagesList) {
				...
				if (pageInfo != null) {
					...
					if (pane.editorPane.getPage() != url) {
						...
						descriptionPanel.setTitleAt(descriptionPanel.indexOfComponent(pane.scrollPane),
								pageInfo.getTitle());
					}
				}
			}
	...
	/**
	 * Extracts the description pages (if not yet done) and opens them in the system
	 * browser
	 */
	...
	/**
	 * Extracts the description pages and opens the required page in the system
	 * browser
	 */
	 ...
	 * 
	 * @return File the temporary directory where files where extracted, null if
	 *         there was any error
	 */
...
//      System.out.println ("Path = "+path);
			...
			if (_isApplet) {
				...
				if (mustExtract) {
					File resource = EjsSimulation.getResourceFile(path);
					if (resource == null)
						JOptionPane.showMessageDialog(_parentComponent,
								ejsRes.getString("Simulation.ErrorWhenOpening") + ":\n  " + _url.getFile());
					else {
						tellTheUserToWait();
...
// --------------------------------------------------------
	...
	// -------------------------------- FKH 20060415 for javascript and java
	// connection
	...
	/*
	 * public void ejsPopup(String url){// works for IE, but it is not working for
	 * netscape String command = "window.open('"+
	 * url+"', 'ejspopup', 'menubar=0,location=0,scrollbars,resizable,width=100,height=100');"
	 * ; ejsEval(command); } protected netscape.javascript.JSObject
	 * htmlWindow=null;//FKH 20060731 public void ejsEval(String command){// call
	 * html window to exec command if(javascriptControlMode(false)){// valid even
	 * for signed applet if(htmlWindow==null)htmlWindow =
	 * netscape.javascript.JSObject.getWindow(javascriptControledApplet);
	 * htmlWindow.eval(command); } } public void ejsCommand(String Args[]){// call
	 * Javascript function from ejs generated simulations
	 * ejsCommand("ejsCommand",Args); } public void ejsCommand(String
	 * JScriptFunctionName,String Args[]){ if(javascriptControlMode(false)){
	 * if(htmlWindow==null)htmlWindow =
	 * netscape.javascript.JSObject.getWindow(javascriptControledApplet);
	 * htmlWindow.call(JScriptFunctionName,Args); } } private boolean
	 * javascriptControlMode(){ return javascriptControlMode(true); } private
	 * boolean javascriptControlMode(boolean localMode){
	 * if(javascriptControledApplet==null)javascriptControledApplet =
	 * model._getApplet(); if(javascriptControledApplet==null ||
	 * isMoodleConnected())return false; if(localMode){ boolean
	 * canUseLocalFile=true;// signed applet try { System.getProperty("user.dir"); }
	 * catch (Exception exc) { canUseLocalFile = false; } return !canUseLocalFile; }
	 * return true; }
	 * 
	 */
	// -------------------------------------------------- END FKH
...
//        System.out.println("Image format supported = "+names[i]);
	 ...
	 */
	/*
	 * public AnimatedGif createAnimatedGif (String _filename, String _element) { if
	 * (view==null) return null; java.awt.Component comp =
	 * view.getComponent(_element); if (comp==null) {
	 * System.err.println("Component not found: "+_element); return null; } return
	 * new AnimatedGif(comp,_filename); }
	 */
...
// --------------------------------------------------------
	...
	public void addMenuEntries(List<Object> _entries) {
		if (popupMenuExtraEntries == null)
			popupMenuExtraEntries = new ArrayList<Object>();
	...
	public void addElementMenuEntries(String elementName, List<Object> _entries) {
		if (disablePopUp)
			return; // don't show "Element Options" in pop up menu.
	...
	public void getPopupMenu(final Component _component, int _x, int _y, String _element) {
		...
		if (popupMenu == null) {
			...
			if (popupMenuExtraEntries != null && popupMenuExtraEntries.size() > 0) {
				for (Object entry : popupMenuExtraEntries)
					addMenuItem(popupMenu, entry);
			...
			// A�adida la condici�n de conexi�n a Moodle (No es necesario que el
			// applet
			// est� firmado cuando manda ficheros a Moodle porque no se hace acceso al
			// disco).
			if (canAccessDisk || isMoodleConnected()) { // if (canAccessDisk) {
				...
				if (videoUtil.isFullClass()) {
					...
					snapshotMenu.add(new AbstractAction(getMenuText("ejs_res:MenuItem.TakeEPSSnapshotWindow")) {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							...
							java.awt.Toolkit.getDefaultToolkit().addAWTEventListener(focusListener,
									AWTEvent.WINDOW_EVENT_MASK);
						}
					});
					snapshotMenu.add(new AbstractAction(getMenuText("tools_res:MenuItem.Video")) {
						public void actionPerformed(ActionEvent e) {
							videoUtil.startVideoTool(getView(), captureElement);
						}
					});
				}
				...
				if (!(isUnderEjs || isLauncherMode || isMoodleConnected()))
					ioStateMenu.add(new AbstractAction(getMenuText("ejs_res:MenuItem.SaveDefaultState")) {
						public void actionPerformed(ActionEvent e) {
							File jarFile = null;
							try {
								...
								String filename = OSPRuntime.chooseFilename(chooser, getParentComponent(), false); // true
																													// =
																													// to
																													// save
								if (filename == null)
			...
			fontMenu.add(new AbstractAction(DisplayRes.getString("DrawingFrame.IncreaseFontSize_menu_item")) {
				public void actionPerformed(ActionEvent e) {
					FontSizer.levelUp();
					for (String windowName : getWindowsList())
						FontSizer.setFonts(getView().getComponent(windowName), FontSizer.getLevel());
				}
			});
			...
			popupMenu.add(new AbstractAction(Memory.getResource("Simulation.AboutThisSimulation")) {
				public void actionPerformed(ActionEvent e) {
					aboutThisSimulation(_component);
				}
			});
...
//            System.err.println("System password is "+systemPassword);
			...
			if (!isUnderEjs && canAccessDisk && !model._isApplet())
				popupMenu.add(new AbstractAction(Memory.getResource("CreateHTMLPage")) {
					public void actionPerformed(ActionEvent e) {
						createHTMLpage(getJarName());
					}
				});
...
//        });
			...
			popupMenu.add(new AbstractAction("OSP " + ToolsRes.getString("MenuItem.Log")) {
				public void actionPerformed(ActionEvent e) {
					...
					if (log != null)
						log.setLocationRelativeTo(parentComponent);
					OSPLog.showLog();
				}
			});
		...
		if (_x >= 0 && _y >= 0)
			popupMenu.show(_component, _x, _y);
		else
			popupMenu.show(_component, (_component.getWidth() - popupMenu.getWidth()) / 2,
					(_component.getHeight() - popupMenu.getHeight()) / 2);
	}
	...
	public void aboutThisSimulation(Component _component) {
		...
		// editorDialog.setSize(editorDialog.getPreferredSize());
		// //res.getDimension("Osejs.CodeDialog.Size"));
	...
	private String getMenuText(String name) {
		String text;
		if (name.startsWith("ejs_res:"))
			text = ejsRes.getString(name.substring(name.indexOf(':') + 1));
		else if (name.startsWith("tools_res:"))
			text = ToolsRes.getString(name.substring(name.indexOf(':') + 1));
		else if (name.startsWith("display_res:"))
			text = DisplayRes.getString(name.substring(name.indexOf(':') + 1));
		else if (name.startsWith("xuggle_res:"))
			text = XuggleRes.getString(name.substring(name.indexOf(':') + 1));
		else
			text = ToolsRes.getString(name);
	...
	private void processMenuTexts(Object object) {
		if (object instanceof JMenu) {
			...
			for (int i = 0, n = menu.getItemCount(); i < n; i++)
				processMenuTexts(menu.getItem(i));
	...
	/**
	 * Returns the list of top level containers of the simulation's view. Subclasses
	...
	/**
	 * Returns the main frame of the simulation's view. Subclasses should overwrite
	 * this method.
	 * 
	 * @return ArrayList
	 */
	...
	/**
	 * Saves the image produced by a component. The name is created from the name of
	 * the simulation class plus a counter
	 * 
	 * @param _element the name of the view element to get the image from
	...
	/**
	 * Saves the image produced by a component. The user will be given the chance to
	 * choose a name.
	 * 
	 * @param _element the name of the view element to get the image from
	 * @return true if the file was correctly saved
	 ...
	 * 
	 * @param _filename the name of a file (either in disk or in memory). Null if
	...
	public boolean saveImage(String _filename, String _element) {
		...
		if (isMoodleConnected() || javascriptControlMode()) {
			if (comp instanceof javax.swing.JFrame)
				comp = ((javax.swing.JFrame) comp).getContentPane();
			else if (comp instanceof javax.swing.JDialog)
				comp = ((javax.swing.JDialog) comp).getContentPane();
			// Generate the image
			...
			if (ctrlEl instanceof SpecialRender)
				((SpecialRender) ctrlEl).render(bi);
			else {
			...
			// Simplemente he cambiado el nombre por defecto del fichero de la imagen para
			// ajustarlo al que
			// ya se pon�a como nombre por defecto para grabar un fichero de texto.
			if (_filename == null)
				_filename = getModelClassname() + ".gif"; // _filename = "default.gif"
			if (isMoodleConnected())
				return moodle.saveImage(_filename, "GIF image", bi) != null;
			// LDLTorre for consistency
			...
			try { // Save it to memory or disk
				...
				if (javascriptControlMode() || _filename.startsWith("ejs:"))
					out = new java.io.ByteArrayOutputStream();
				else
					out = new java.io.FileOutputStream(_filename);
				boolean result = true;
				if (supported)
					result = javax.imageio.ImageIO.write(bi, format, out);
				else
					result = videoUtil.writeGIF(out, bi); // use GIFEncoder
				out.close();
				if (!result)
					return false;
	...
	public void processArguments(String[] _args) {
		...
		if (applet != null) {
			try {
				...
				if (arg0 != null)
					stateRead = readState(arg0, applet.getCodeBase());
			} catch (Exception exception) {
				exception.printStackTrace();
			}
		} else {
...
//      else System.out.println ("No default state to read");
		...
		if (applet == null) { // decide whether or not to show the description
			...
			if (org.opensourcephysics.display.OSPRuntime.isLauncherMode())
				showIt = false;
			else
				try {
			...
			; // Do not complain
			if (showIt && showDescriptionOnStart)
				showDescription();
		}
	...
	/**
	 * Gets the default list used when saving or reading the state. If null, the
	 * whole state is saved using reflection.
	 */
	...
	/**
	 * Sets the default list to use when saving or reading the state. If not set,
	 * the whole state is saved using reflection.
	 * 
	 * @param _list ArrayList the list of variables that define the state to save or
	 *              read
	 */
	...
	/**
	 * Sets the default list to use when saving or reading the state If not set, the
	 * whole state is saved using reflection.
	 * 
	 ...
	 * 
	 * @param _filename the name of a file, either in disk , in memory (if given the
	 *                  prefix "ejs:") or from a url (if given the prefix "url:") If
	 *                  running as an applet, the codebase of the applet is used for
	 *                  a non-qualified file (one that does not starts explictly
	 *                  with "http:"). If file is null the user will be prompted to
...
//    return readVariables (_filename,theCodebase,stateVariablesList);
	 ...
	 * 
	 * @param _filename the name of a file, either in disk , in memory (if given the
	 *                  prefix "ejs:") or from a url (if given the prefix "url:")
...
// --------------------------------------------------------
	...
	/**
	 * Reads the state of the specified variables of the model either from a file on
	 * the disk, from memory or from a url location.
	 * 
...
//    return readVariables (_filename,_codebase,toArrayList(_varList));
	 ...
	 * the disk, from memory or from a url location.
	 * <p>
	 * If the name of the file starts with the prefix "ejs:", then the state of the
	 * model will be read from a memory file that must have been created previously
	 * with the corresponding call to saveVariables().
	 * </p>
	 * <p>
	 * If the name of the file starts with "url:" it will be considered a url
	 * location and the method will attempt to read the file from this url (either
	 * locally or through the network).
	 * </p>
	 * <p>
	 * If the codebase is non-null the file is considered to be a network file on
	 * that codebase.
	 * </p>
	 * <p>
	 * In all other cases, the file will be considered to be on a disk. If the
	 * filename is null, the user will be prompted to choose a file to read from.
	 * </p>
	 * <p>
	 * If the name of the file ends in ".xml" the data is expected to be in XML
	 * form. Otherwise it is considered to be binary.
	 * </p>
	 * <p>
	...
	public boolean readVariables(String _filename, List<String> _varList) {
		if (model == null)
			return false;
		// LDLTorre for Moodle support
		// A�adida una nueva posibilidad si estamos en Moodle. Si _filename es null,
		// cargamos una
		// interfaz que muestra una lista de aquellos ficheros contenidos en Moodle y
		// generados por
		 ...
		 * _filename = OSPRuntime.chooseFilename(chooser,popupTriggeredBy,false); //
		 * false = to read if (_filename==null) return false; }
		 */
		if (isMoodleConnected()) {
			// When working in Moodle, it checks if filename is null. In that case, it
			// connects to Moodle
			// php and obtains the list of xml files, created with this EJS lab, as a
			// string.
			_filename = moodle.readXML(_filename);
			if (_filename.equals("url:"))
				return false;
			...
			if (_filename.toLowerCase().endsWith(".xml")) { // Read XML
				...
				if (urlConnection != null) {
					...
					if (_filename.startsWith("ejs:"))
						...
						xmlString = moodle.readXML(_filename);
					else
						in = reader;
				}
				if (in != null) {
...
//            System.err.println (" Value read = "+objectRead+ " (type = "+fields[i].getType()+")");
	 ...
	 * either to a file on the disk or to memory. If the list is null, it saves all
	 * the variables of the model.
	 * <p>
	 * If the name of the file starts with the prefix "ejs:", then the state of the
	 * model will be saved to memory, otherwise it will be saved to disk.
	 * </p>
	 * <p>
	 * If the name of the file ends with ".xml", then the state of the model will be
	 * saved in XML form.
	 * </p>
	 * <p>
	 * Security considerations apply when running the simulation as an applet.
	 ...
	 * @param _varList  the list of variables to save. If null, all variables are
	 *                  saved
	 * @return true if the file was correctly saved
	 */
	public boolean saveVariables(String _filename, List<String> _varList) {
		...
		if (_filename == null) { // Choose a filename, preferably an ".xml" file
			...
			// Introducido un if para hacer la verificaci�n de conexi�n a Moodle a fin
			// de evitar en este
			// caso la aparici�n del panel de navegaci�n por el disco duro. Tambi�n se
			// introduce un nombre
			// por defecto para cuando no se est� conectado a Moodle.
			if (isMoodleConnected())
				_filename = getModelClassname() + "_Variables.xml";
			else {
				JFileChooser chooser = OSPRuntime.createChooser("XML", new String[] { "xml" });
						...
						if (command.equals(JFileChooser.APPROVE_SELECTION)) {
							...
							if (accesory.saveAsXML()) { // must end with ".xml"
								if (!selectedFilename.toLowerCase().endsWith(".xml")) {
									theFileChooser.setSelectedFile(
											new File(selectedFile.getParent(), selectedFilename + ".xml"));
									nameChanged = true;
								}
							} else { // anything but ".xml"
								if (selectedFilename.toLowerCase().endsWith(".xml")) {
									theFileChooser.setSelectedFile(
											new File(selectedFile.getParent(), selectedFilename + ".bin"));
									nameChanged = true;
								}
							}
							if (nameChanged) {
								JOptionPane.showMessageDialog(popupTriggeredBy,
										ejsRes.getString("Simulation.XMLFileRenamed") + ":\n  "
		...
		try {
			...
			if (saveAsXML)
				control = new XMLControlElement(this.getClass()); // save as XML
			else { // Save in binary form
				// FKH 20060415 add javascriptcontrolMode()
				if (javascriptControlMode() || _filename.startsWith("ejs:") || isMoodleConnected())
...
//            System.err.println ("Saving variable "+fields[i].getName()+" to "+objectToSave+ " (type = "+fields[i].getType()+")");
			...
			if (control != null) { // Write the XML
				 ...
				 * else if (isMoodleConnected()) return moodle.saveXML(_filename, "XML file",
				 * control.toXML()) != null; else writer = new java.io.FileWriter (_filename);
				 * control.write(writer); if (_filename.startsWith("ejs:"))
				 * memory.put(_filename,((java.io.CharArrayWriter) writer).toCharArray());
				 */
				if (isMoodleConnected())
					return moodle.saveXML(_filename, "XML file", control.toXML()) != null;
				if (_filename.startsWith("ejs:"))
					writer = new java.io.CharArrayWriter();
				else
					writer = new java.io.FileWriter(_filename);
				...
				if (out != null) {
					...
					/*
					 * if (_filename.startsWith("ejs:"))
					 * memory.put(_filename,((ByteArrayOutputStream) out).toByteArray()); else if
					 * (isMoodleConnected ()) return moodle.saveBinary(_filename,"Binary data",(
					 * (ByteArrayOutputStream) out).toByteArray())!=null; else
					 * if(javascriptControlMode())
					 * javascriptControledApplet.setStateByteArray(((java.io.ByteArrayOutputStream)
					 * out).toByteArray());
					 */
					if (isMoodleConnected())
						return moodle.saveBinary(_filename, "Binary data",
								((ByteArrayOutputStream) out).toByteArray()) != null;
					if (_filename.startsWith("ejs:"))
						memory.put(_filename, ((ByteArrayOutputStream) out).toByteArray());
					else if (javascriptControlMode())
						javascriptControledApplet
								.setStateByteArray(((java.io.ByteArrayOutputStream) out).toByteArray());
					// LDLTorre for Moodle support
				}
			}
			return true;
	...
	public boolean saveDefaultStateToJar(File _jarFile, String _filenames) {
		...
		try {
			...
			if (!savedOk) {
				JOptionPane.showMessageDialog(getParentComponent(), "Cannot save data file",
						"Could not create temp file", JOptionPane.ERROR_MESSAGE);
				return false;
			}
//      System.out.println ("Saved OK "+tempFile.getAbsolutePath());
			...
			if (_filenames != null) {
				...
				while (tkn.hasMoreTokens()) {
					...
					if (!isDisplayable(extraFileName))
						continue;
...
//          System.err.println ("res = "+res+" , FIle = "+extraFile);
					...
					if (extraFile == null || !extraFile.exists()) {
						if (res == null || res.openReader() == null)
							JOptionPane.showMessageDialog(getParentComponent(),
									ejsRes.getString("Simulation.CantFindExtraFile") + " " + extraFileName,
									ejsRes.getString("Simulation.IgnoringExtraFile"), JOptionPane.WARNING_MESSAGE);
						else
							JOptionPane.showMessageDialog(getParentComponent(),
									ejsRes.getString("Simulation.ExtraFileAlreadyIn") + " " + extraFileName + "\n"
											+ ejsRes.getString("Simulation.RenameDocument"),
									ejsRes.getString("Simulation.IgnoringExtraFile"), JOptionPane.WARNING_MESSAGE);
					} else
			...
			while (entry != null) {
				...
				if (lowercaseName.startsWith("meta-inf/") && (lowercaseName.endsWith(".sf")
						|| lowercaseName.endsWith(".rsa") || lowercaseName.endsWith(".dsa"))) {
				...
				for (File extraFile : extraFilesSet) {
					if (extraFile.getName().equals(name)) {
						int option = JOptionPane.showConfirmDialog(getParentComponent(),
								...
								"Conflict with existing file", JOptionPane.YES_NO_OPTION);
...
//      control = new XMLControlElement(this.getClass());
			...
			// Add the extra files
			for (File extraFile : extraFilesSet)
				addFileToJar(jarOut, extraFile.getName(), extraFile);
...
//      jarOut.closeEntry();
			...
			if (htmlCheckBox.isSelected())
				createHTMLpage(newJarFile.getName());
			return true;
		} catch (java.lang.Exception ioe) {
...
//    }
	...
	/**
	 * Returns the state of the variables of the model specified in the given list a
	 * a text with XML format.
	 * 
	 * @param _varList a String with the list of variables separated by commas ','
	 *                 or semicolons ';'
	...
	/**
	 * Returns the state of the variables of the model specified in the given list a
	 * a text with XML format. If the list is null, it saves all the variables of
	 * the model.
	 * <p>
	 * The state of the model is saved by writing to disk all its public fields
	 * which implement the java.io.Serializable interface. This includes primitives
...
// --------------------------------------------------------
	...
	/**
	 * Saves a String with user-defined info either to a file on the disk or to
	 * memory.
	 * <p>
	 * If the name of the file starts with the prefix "ejs:", then the state of the
	 * model will be saved to memory, otherwise it will be dumped to disk.
	 * </p>
	 * <p>
	 * Security considerations apply when running the simulation as an applet.
	 * <p>
		...
		try {
			...
			// LDLTorre for Moodle support
				...
				if (_filename.toLowerCase().endsWith(".xml"))
					return moodle.saveXML(_filename, "XML file", _text) != null;
	...
	/**
	 * Saves user generated info to a file. Equivalent to saveText
	 * (_filename,_info.toString());
	 * 
	 ...
	 * 
	 * @param _filename the name of a file (either in disk, in memory or a url)
	 * @param _type     the type of files you want to search for when embedded in
	 *                  Moodle
	 * @return String The string read, or null if failed
	 */
	 ...
	 * </p>
	 * <p>
	 * If the name of the file starts with the prefix "ejs:", then the info string
	 * will be read from a memory file.
	 * </p>
	 * <p>
	 * If the name of the file starts with "url:" it will be considered a url
	 * location and the method will attempt to read the file from this url (either
	 ...
	 * </p>
	 * <p>
	 * If the name of the file does not start with any of those prefixes, then it
	 * will be considered to be a file. Security considerations apply when running
	 * the simulation as an applet.
	 * </p>
	 * <p>
	 * 
	 * @param _filename the name of a file (either in disk, in memory or a url)
	...
	public String readText(String _filename, String _type, java.net.URL _codebase) {
		...
		if (isMoodleConnected() || _filename.startsWith("url:")) {
			...
			if (isMoodleConnected()) {
				if (_filename.toLowerCase().endsWith(".xml"))
					url = moodle.readXML(_filename);
				else
					url = moodle.readText(_filename, _type);
				if (url.equals("url:"))
					return null;
			} else if (_filename.startsWith("url:")) {
