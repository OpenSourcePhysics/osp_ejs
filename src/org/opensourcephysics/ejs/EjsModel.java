...
//  private boolean hasBeenReset = false;
	...
	static public final TranslatorUtil _getTranslatorUtil() {
		return __translatorUtil;
	}

	static public final GraphicsConfiguration getGraphicsConfiguration() {
	 ...
	 * 
	 * /** Prints a string on any of the printers attached to this computer. Allows
	 * to specify the margins
	 *
	 * static public void _printerJob (String title, String output, int fontSize,
	 * double top, double left, double bottom, double right) { printUtil.printerJob
	 * (title, output, fontSize, top, left, bottom, right); }
	 */
	...
	/**
	 * Returns a String property (as defined in the Translation tool) in the current
	 * Locale. If not found, the property is returned
	 * 
	 * @return
	 */
...
//  public String _getStringProperty(String _property, String _default) { return __translatorUtil.translateString(_property,_default); }
	...
	public EjsSimulation getSimulation() {
...
//  abstract public void _initializeSolvers();
	...
	/**
	 * Actions that the model may want to do after the view is updated. For
	 * instance, a Tracker model element will read the (x,y) position of a view
	 * element
	 */
	...
	public void _setDelay(int _delay) {
		_getSimulation().setDelay(_delay);
	}
...
//    _external.reset(); 
	...
	public void _setSPD(int _steps) {
		_getSimulation().setStepsPerDisplay(_steps);
	}

	public void _setUpdateView(boolean _update) {
		_getSimulation().setUpdateView(_update);
	}

	public void _showDescriptionAtStartUp(boolean _show) {
		_getSimulation().showDescriptionAtStartUp(_show);
	}

	public void _showDescription(boolean _show) {
		_getSimulation().showDescription(_show);
	}

	public java.net.URL _getDescriptionPageURL(String pageName) {
		return _getSimulation().getDescriptionPageURL(pageName);
	}
	 ...
	 * files required by the model to run (and included in the jar file)
	 * 
	 * @return
	 */
	public java.util.Set<String> _getClassEjsResources() {
	 ...
	 * 
	 * @return The base directory from which the model was compiled. May not start
	 *         with a leading "/".
	 */
	...
	public String[] _getArguments() {
		return __theArguments;
	}

	public boolean _hasDefaultState() {
		return _getSimulation().hasDefaultState();
	}

	public boolean _readDefaultState() {
		return _getSimulation().readDefaultState();
	}

	public boolean _saveDefaultStateToJar() {
		return _saveDefaultStateToJar(null);
	}

	public boolean _saveDefaultStateToJar(String filenames) {
		if (_getSimulation().isUnderEjs())
			return false;
		File jarFile = null;
		try {
			...
			String filename = OSPRuntime.chooseFilename(chooser, _getSimulation().getParentComponent(), false); // true
																												// = to
																												// save
			if (filename == null)
				return false;
	...
	public boolean _saveState(String _filename) {
		return _getSimulation().saveState(_filename);
	}

	public boolean _readState(String _filename) {
		return _getSimulation().readState(_filename);
	}
	...
	public boolean _readVariables(String _filename, java.util.List<String> _varList) {
		...
		if (__theApplet != null)
			codebase = __theApplet.getCodeBase();
	...
	public boolean _saveImage(String _filename, String _element) {
		return _getSimulation().saveImage(_filename, _element);
	}

	public void _alert(String _panel, String _title, String _message) {
		if (_getView() != null)
			JOptionPane.showMessageDialog(_getView().getComponent(_panel), _message, _title,
					JOptionPane.INFORMATION_MESSAGE);
		else
			JOptionPane.showMessageDialog(_getSimulation().getParentComponent(), _message, _title,
					JOptionPane.INFORMATION_MESSAGE);
	}
	...
	public boolean _isMoodleConnected() {
		return _getSimulation().isMoodleConnected();
	}
	...
	public String _getParameter(String _name) {
		if (__theApplet != null)
			return __theApplet.getParameter(_name);
		if (__theArguments == null)
			return null;
		for (int i = 0; i < __theArguments.length; i++) {
			if (__theArguments[i].equals("-" + _name) && (i + 1) < __theArguments.length)
				return __theArguments[i + 1];
		}
		return null;
	}
	...
	static protected boolean _common_model_initialization() {
		...
		try { // in case of security problems
			if (System.getProperty("osp_ejs") != null) { // Running under EJS
				underEjs = true;
				org.opensourcephysics.ejs.EjsSimulation
						.setPathToLibrary("/Users/wochristian/Desktop/EjsS_5.3/bin/config/");
		...
		if (!pathsSet) {
			org.opensourcephysics.ejs.EjsSimulation.setPathToLibrary("/Users/wochristian/Desktop/EjsS_5.3/bin/config/");
			// This is for classes (such as EjsMatlab) which needs to know where the library
			// is
		}
		if (!underEjs) {
	...
	// -------------------------------- FKH 20060903 for javascript and java
	// connection
	/*
	 ...
	 * 
	 * public void _ejsCommand (String[] _args) { _getSimulation().ejsCommand
	 * (_args); }
	 * 
	 * public void _ejsCommand (String _function_name,String[] _args) {
	 * _getSimulation().ejsCommand (_function_name,_args); }
	 * 
	 */
	// -------------------------------------------------- END FKH
...
//  }
	...
	// abstract public String _externalInitCommand(String _applicationFile);
	// //Gonzalo 090610
...
//   */
	// abstract public void _externalSetValues(boolean _any, ExternalApp
	// _application); //Gonzalo 090610

//  /**
//   * Get the value of all the variables in the external application.
//   * If _any is true, then get the values of all sessions.
//   */
	// abstract public void _externalGetValues(boolean _any, ExternalApp
	// _application); //Gonzalo 090610
